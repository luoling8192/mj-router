...
async def process_image_request(task_id: str, request: ImageRequest) -> None:
    ...
    try:
        ...
        task.result_url = result["data"][0]["url"] if request.provider.value == "openrouter" else result["image_url"]
...
async def generate_image(request: ImageRequest, background_tasks: BackgroundTasks) -> ImageResponse:
